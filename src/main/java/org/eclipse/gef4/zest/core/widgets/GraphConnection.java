...
public class GraphConnection extends GraphItem {
	...
	void removeFigure() {
		...
		if (sourceContainerConnectionFigure != null) {
			sourceContainerConnectionFigure.getParent()
					.remove(sourceContainerConnectionFigure);
			sourceContainerConnectionFigure = null;
		}
		if (targetContainerConnectionFigure != null) {
			targetContainerConnectionFigure.getParent()
					.remove(targetContainerConnectionFigure);
	...
	public String toString() {
		...
		buffer.append(
				destinationNode != null ? destinationNode.getText() : "null");
	...
	public void setCurveDepth(int depth) {
		if (this.curveDepth == 0 && depth != 0
				|| this.curveDepth != 0 && depth == 0) {
	...
	public void setVisible(boolean visible) {
		if (getSource().isVisible() && getDestination().isVisible()
				&& visible) {
	...
	public void setDirected(boolean directed) {
		if (directed) {
			...
			setConnectionStyle(
					connectionStyle & (-1 - ZestStyles.CONNECTIONS_DIRECTED));
		}
	}
		...
		if (getSource() == getDestination()) {
			...
			labelLocator = new MidpointLocator(connectionFigure, 0) {
				protected Point getReferencePoint() {
					...
					p.y = (int) (getConnection().getPoints()
							.getPoint(getIndex()).y - (curveDepth * 1.5));
			...
			applyConnectionRouter(connectionFigure);
			sourceAnchor = new RoundedChopboxAnchor(getSource().getNodeFigure(),
					8);
			targetAnchor = new RoundedChopboxAnchor(
					getDestination().getNodeFigure(), 8);
			labelLocator = new MidpointLocator(connectionFigure, 0);
		}
	...
	class InternalConnectionLayout implements ConnectionLayout {
		...
		void applyLayout() {
			// commented out to fix reappearing invisible connections on setInput
			/*
	...
	void applyDecoration(PolylineArcConnection conn) {
		...
		if (decorator == null) {
			decorator = ((connectionStyle
					& ZestStyles.CONNECTIONS_DIRECTED) > 0)
							? getGraphModel()
									.getDefaultDirectedConnectionDecorator()
							: getGraphModel().getDefaultConnectionDecorator();
		}
	...
	public void setConnectionDecoration(
