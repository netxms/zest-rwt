...
public class Graph extends FigureCanvas implements IContainer {
	...
	public Graph(Composite parent, int style) {
		...
		this.getLightweightSystem().setEventDispatcher(
				new SWTEventDispatcher() {
					public void dispatchMouseMoved(
						...
						if ((getMouseTarget() == null) || (nodeDragging && (getMouseTarget() instanceof IDecorationFigure)))
						{
							setMouseTarget(getRoot());
						}
						if ((me.stateMask & SWT.BUTTON_MASK) != 0) 
						{
							// this hack allows object move when grid is shown
							// (issue #337)
							// opaque attribute used to detect grid figure as it
							// is
							// currently only figure with opaque set to false
							if ((getMouseTarget() instanceof Figure)
									&& !((Figure) getMouseTarget()).isOpaque()) {
		...
		zestRootLayer.addMouseListener(dragSupport);
		zestRootLayer.addMouseMotionListener(dragSupport);
	...
	/**
	 * Get root layer for zest objects (nodes, connections, etc.)
	 * 
	 * @return
	 */
	...
	private class DragSupport implements MouseMotionListener,
		...
		public void mouseDragged(org.eclipse.draw2d.MouseEvent me) {
			...
			if ((me.getSource() instanceof ZestRootLayer) || (me.getSource() instanceof IDecorationFigure))
				getRootLayer().translateToParent(mousePoint);
			...
			if (!selectedItems.isEmpty() || draggedSubgraphFigure != null) 
			{
				...
				for (Iterator selectionIterator = selectedItems.iterator(); selectionIterator
					...
					if ((item.getItemType() == GraphItem.NODE)
						...
						if (!(me.getSource() instanceof ZestRootLayer) && !(me.getSource() instanceof IDecorationFigure))
						{
							getRootLayer().translateToRelative(pointCopy);
						}

						pointCopy.performScale(1 / getRootLayer().getScale());
		...
		private Point getRelativeLocation(IFigure figure) 
		{
			...
			getRootLayer().translateToParent(location);
			location.x -= dragStartLocation.x;
			location.y -= dragStartLocation.y;
			return location;
		}
	 ...
	 * 
	 * @param draggingEnabled true to enable dragging
	 */
